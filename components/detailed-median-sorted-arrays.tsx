...
export function DetailedMedianSortedArrays() {
  // 修改状态定义
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-100 to-purple-100 p-8">
      ...
      {/* 添加数组输入和控制按钮部分 */}
      <div className="bg-white rounded-xl shadow-xl p-8 w-full max-w-4xl mb-8">
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2 text-lg border-2 border-indigo-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
          <div>
            <label className="block text-lg font-medium text-gray-700 mb-2">
      ...
      {/* 原有的可视化部分保持不变 */}
      <div className="bg-white rounded-xl shadow-xl p-8 w-full max-w-4xl space-y-8">
        {/* 输入数组部 */}
          ...
          <div className="border rounded-lg p-6 shadow-sm">
            ...
            <div className="flex relative pt-12">
              {nums1.map((num, index) => (
                <motion.div
                  key={`nums1-${index}`}
                  className={`w-12 h-12 flex items-center justify-center border border-gray-300 rounded-md m-1 ${
                    index < i ? "bg-green-200" : "bg-white"
                  }`}
              ...
              >
                <span className="text-blue-500 font-bold mb-1">i</span>
          ...
          <div className="border rounded-lg p-6 shadow-sm">
            ...
            <div className="flex relative pt-12">
              ...
              <motion.div
                ...
                transition={{ duration: 0.5 }}
              >
                <span className="text-red-500 font-bold mb-1">j</span>
          ...
          <div className="flex flex-wrap">
            <AnimatePresence>
              {merged.map((num, index) => (
                <motion.div
                  key={`merged-${index}`}
                  className="w-12 h-12 flex items-center justify-center border border-gray-300 rounded-md m-1 bg-blue-200"
          ...
          <motion.p
            ...
            dangerouslySetInnerHTML={{
              __html: explanation.replace(
                /<code>(.*?)<\/code>/g,
                '<span class="bg-yellow-200 text-gray-800 px-1 rounded">$1</span>'
              ),
            }}
          />
        </div>
      ...
      {/* 步骤进度 */}
      <div className="mt-8 px-4 py-2 bg-white rounded-full shadow-sm text-gray-600">
        步骤: {step} / {totalSteps}
      </div>
    </div>
  );
